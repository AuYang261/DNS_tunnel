...
void PacketAnalyzer::analysePacket(pcpp::RawPacket* packet) {
    ...
    std::cout << "dns_features.num_answers: " << dns_features.num_answers
              << std::endl;
    std::cout << "dns_features.num_authority: " << dns_features.num_authority
              << std::endl;
    ...
    if (start_timestamp.tv_sec == 0 && start_timestamp.tv_nsec == 0)
        [[unlikely]] {
    ...
    printf("timestamp: %ld.%09ld\n", dns_features.timestamp.tv_sec,
           dns_features.timestamp.tv_nsec);
}